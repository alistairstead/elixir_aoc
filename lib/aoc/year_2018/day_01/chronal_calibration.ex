defmodule Aoc.Year2018.Day01.ChronalCalibration do
  ...
  *What is the first frequency your device reaches twice?*
  ...
  def part_2(input) do
    ...
    |> String.split()
    |> Stream.cycle()
    |> Enum.reduce_while({0, MapSet.new([0])}, fn x, {last, visited} ->
      result = last + String.to_integer(x)

      if(MapSet.member?(visited, result)) do
        {:halt, result}
      else
        {:cont, {result, MapSet.put(visited, result)}}
      end
    end)
  end
end
